...
typedef union _L2PF0_BCR {
    struct {
        uint32_t ScalaLoadSupportEn:1;        /**< bit: 0 - Enable hardware prefetcher */
        uint32_t CrossPageOptmDis:1;       /**< bit: 1 - Only works when CrossPageEn ===0  */
        uint32_t Dist:6; /**< bit: 2..7 - Initial prefetch distance */
        uint32_t MaxAllowedDist:6;       /**< bit: 8..13 - Maximum allowed Distance: 1<< maxAllowedDistance */
        uint32_t LinToExpThrd:6; /**< bit: 14..19 - Linear to exponential threshold */
        uint32_t AgeOutEn:1;        /**< bit: 20 - Ageout mechanism enable */
        uint32_t NumLdsToAgeOut:7;       /**< bit: 21..27 - Num of non-matching loads to edge out an entry */
        uint32_t CrossPageEn:1; /**< bit:  28 - Enable prefetches to cross pages */
...
#define L2PF0_BCR_SCALALOADSUPPORTEN_Msk    (1U << L2PF0_BCR_SCALALOADSUPPORTEN_Pos)
...
typedef union _L2PF0_L2UBCR {
    struct {
        uint32_t QFullnessThrd:4; /**< bit: 0..3 - Threshold # of Fullness (L2 MSHRs used/ total available) to stop sending hits */
        uint32_t HitCacheThrd:5; /**< bit 4..8 - (1 << hitCacheThrd) of CacheHits for evicting SPF entry */
        uint32_t HitMSHRThrd:4; /**> bit 9..12 - Threshold # of MSHR hits for increasing SPF distance */
        uint32_t Window:6; /**< bit: 13..18 - Size of the comparison window for address matching */
    } b;                        /**< Structure used for bit access */
    uint32_t w;                 /**< Structure used for word access */
} sifive_l2pf0_l2ubcr;
