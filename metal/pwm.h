...
struct metal_pwm_vtable {
    ...
    int (*set_freq)(struct metal_pwm *pwm, unsigned int freq);
    ...
    int (*trigger)(struct metal_pwm *pwm, metal_pwm_run_mode_t mode);
 ...
 * @return 0 If no error.*/
inline int metal_pwm_set_freq(struct metal_pwm *pwm, unsigned int freq) {
    return pwm->vtable->set_freq(pwm, freq);
}
...
inline unsigned int metal_pwm_get_freq(struct metal_pwm *pwm) {
    return pwm->vtable->get_freq(pwm);
}

/*! @brief Starts a PWM instance in selected run mode (continuous/one shot).
 * @param pwm PWM device handle.
 * @return 0 If no error.*/
inline int metal_pwm_trigger(struct metal_pwm *pwm, metal_pwm_run_mode_t mode) {
    return pwm->vtable->trigger(pwm, mode);
}
