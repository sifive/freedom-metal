...
struct mee_pmp_vtable {
    void (*init)(struct mee_pmp *pmp);
    int (*set_region)(struct mee_pmp *pmp, unsigned int region, struct mee_pmp_config config, size_t address);
    int (*get_region)(struct mee_pmp *pmp, unsigned int region, struct mee_pmp_config *config, size_t *address);
};

struct mee_pmp {
    const struct mee_pmp_vtable *vtable;
};
...
/* Initializes the PMP. */
inline void mee_pmp_init(struct mee_pmp *pmp) { pmp->vtable->init(pmp); }

/* Configure a PMP region */
inline int mee_pmp_set_region(struct mee_pmp *pmp, unsigned int region, struct mee_pmp_config config, size_t address) { return pmp->vtable->set_region(pmp, region, config, address); }

/* Get the configuration for a PMP region */
inline int mee_pmp_get_region(struct mee_pmp *pmp, unsigned int region, struct mee_pmp_config *config, size_t *address) { return pmp->vtable->get_region(pmp, region, config, address); }
