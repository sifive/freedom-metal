...
inline void __mee_shutdown_exit(const struct mee_shutdown *sd, int code) __attribute__((noreturn));
inline void __mee_shutdown_exit(const struct mee_shutdown *sd, int code) { sd->vtable->exit(sd, code); }
