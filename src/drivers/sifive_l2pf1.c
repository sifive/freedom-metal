...
static void __sifive_l2pf1_import_basic_control_reg(sifive_l2pf1_config *config, const uint32_t val, const uint32_t dist_bits_mask) {
    config->ScalarLoadSupportEn = (val & REG_MASK_BITWIDTH_1);
    config->Dist = ((val >> REG_BITSHIFT_2) & dist_bits_mask);
    config->MaxAllowedDist = ((val >> REG_BITSHIFT_8) & dist_bits_mask);
    config->LinToExpThrd = ((val >> REG_BITSHIFT_14) & dist_bits_mask);
...
#else
    /* Default basic control register initial configuration (0x15811). */
...
#else
    /* Default L2 user bits control register initial configuration (0x38c84e). */
