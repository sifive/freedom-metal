...
#define METAL_PWM_LOG(x...) printf(x)
#else
#define METAL_PWM_LOG(x...)
#endif
...
static void post_rate_change_callback(void *priv) {
    ...
    if (pwm->freq != 0) {
        ...
        gpwm->vtable->set_freq(gpwm, pwm->freq);
...
static int __metal_driver_sifive_pwm0_set_freq(struct metal_pwm *gpwm,
    ...
    if ((clock != NULL) && (gpwm != NULL)) {
        ...
        METAL_PWM_LOG("PWM requested freq:%u set freq:%u \n", freq, pwm->freq);
        METAL_PWM_LOG("CPU Clk:%u Prescale:%u Count:%u \n", clock_rate,
                      prescale, count);
    }
    return ret;
}
...
static unsigned int
__metal_driver_sifive_pwm0_get_freq(struct metal_pwm *gpwm) {
    struct __metal_driver_sifive_pwm0 *pwm = (void *)gpwm;
    unsigned int freq = 0;
