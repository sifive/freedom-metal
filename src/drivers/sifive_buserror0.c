...
#include <meta/generated/sifive_buserror0.h>
#include <stddef.h>
#include <stdint.h>
...
static inline uintptr_t get_index(struct metal_beu beu) {
    return beu.__beu_index;
}
...
int metal_buserror_set_event_enabled(struct metal_buserror beu,
                                     ...
                                     bool enabled) {
    uintptr_t base = dt_beu_data[get_index(beu)].base_addr;
...
metal_buserror_get_platform_interrupt_parent(struct metal_buserror beu) {
    return dt_beu_data[get_index(beu)].interrupt_parent;
}

int metal_buserror_get_platform_interrupt_id(struct metal_buserror beu) {
    return dt_beu_data[get_index(beu)].interrupt_id;
}
