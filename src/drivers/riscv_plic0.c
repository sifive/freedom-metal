    ...
    if (!plic_state[get_index(plic)].init_done) {
        for (int parent = 0; parent < __METAL_PLIC_NUM_PARENTS; parent++) {
            ...
            for (int i = 0; i < __METAL_DT_RISCV_PLIC0_MAX_INTERRUPTS; i++) {
                ...
                if (i < num_interrupts) {
                    riscv_plic0_set_priority(intc, i, 0);
...
unsigned int
riscv_plic0_get_priority(struct metal_interrupt plic,
                                        int id) {
    unsigned long base = dt_intc_data[get_index(plic)].base;
...
metal_affinity
riscv_plic0_affinity_enable(struct metal_interrupt plic,
...
metal_affinity
riscv_plic0_affinity_disable(struct metal_interrupt plic,
                                            metal_affinity bitmask, int id) {
    metal_affinity ret = {0};
    int context;
...
metal_affinity riscv_plic0_affinity_set_threshold(
...
unsigned int riscv_plic0_affinity_get_threshold(
